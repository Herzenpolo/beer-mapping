...
class CocktailRecipe extends Component {
  ...
  componentDidMount = () => {
    Axios.get(cocktailUrl + "s=" + this.props.match.params.id, cocktailKey)
      .then((res3) =>
        this.setState({
          drinkRecipe: res3.data.drinks[0],
          drinkName: res3.data.drinks[0].strDrink,
        })
      )
    ...
    for (let key in this.state.drinkRecipe) {
      if (key.includes("strIngredient")) {
        if (this.state.drinkRecipe[key]) {
          ...
          ingredients.push(
            <h4 className="drink-ingredients">{this.state.drinkRecipe[key]}</h4>
          );
        }
      }
    }
  ...
  getInstructions = () => {
    ...
    return <p className="drink-instructions">{instructions}</p>;
  };
